    ...
    switch (m_phase) {
        case ArtisanPhase::Idle: {
            ...
            if (SendArtisanPacket(m_currentTargetId, materialId, GetArtisanAction(m_materialType))) {
                m_sentCount++;
                spdlog::debug("[artisan] Sent #{} target={} mat={} prevType={} prevDur={}/{}",
                    m_sentCount, m_currentTargetId, materialId, m_prevTypeId,
        ...
        case ArtisanPhase::WaitUpgradeResult: {
            ...
            const OBJID curTypeId = item->GetTypeID();
            const int curDur = item->GetDurabilityRaw();

            // Session 14 [REAL LEVEL FIELD]: live-confirmed via before/after
            // dumps that a successful Artisan Wind upgrade changes the
            // item's TYPE ID (and name) to a different item entirely —
            // e.g. ApeCoat(133443) -> Gambeson(133453) — not a stat byte on
            // the same instance. GetPlus() never reflects this at all,
            // which is why the old plus-based check never once fired.
            if (curTypeId != m_prevTypeId) {
                m_successCount++;
                spdlog::info("[artisan] SUCCESS: item {} type {} -> {} ('{}')",
                    m_currentTargetId, m_prevTypeId, curTypeId, item->GetName());
                m_phase = ArtisanPhase::Idle;
                // Durability stays full (relative to the NEW type's own max)
                // on success, so staying on this item (m_currentTargetId
                // left set) just re-enters Idle and sends the next material
                // immediately — unless the user wants to bank this win and
                // move to the next item instead.
                if (m_stopOnLevelChange)
                    m_currentTargetId = 0;
                return;
            }

            // Only a real failure if the type DIDN'T change — a type change
            // can itself raise or lower the new tier's max durability
            // (observed both directions live), which would otherwise get
            // misread as a failure by a durability-only comparison.
